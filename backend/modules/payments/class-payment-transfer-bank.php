...
if (!class_exists('Bank_Transfer')) {
    ...
    class Bank_Transfer extends Payment_Template
    {
        ...
        public function __construct()
        {
            ...
            $settings = !empty(get_option('lsdcommerce_payment_settings')) ? get_option('lsdcommerce_payment_settings') : null;
        ...
        private function default_settings()
        {
            ...
            if (!isset($payment_data[$this->id]) || $payment_data[$this->id] == null) { // Empty and Not Isset
                ...
                $payment_data[$this->id] = array(
                    'master' => $this->id,
                    'pointer' => $this->id,
                    'name' => __('Bank Custom One', 'lsdcommerce'),
                    'description' => 'Semua Pembayaran dengan Kode QR',
                    'logo' => LSDC_URL . 'backend/assets/images/payment/custom.png',
                    'group' => 'bank_transfer',
                    ...
                    'instruction' => __('Please transfer to this bank account according to the total', 'lsdcommerce'),
        ...
        {
            if (isset(get_option('lsdcommerce_payment_settings')[$this->id])) {
        ...
        public function manage($payment_id)
        {
            ...
            <div id="<?php echo $payment_id; ?>_content" class="payment-editor">
                    ...
                    <form>
                        ...
                        <div class="form-group">
                            <label class="form-label" for="pointer">
                                <?php _e('Custom ID (Required)', 'lsdcommerce');?>
                            </label>
                            ...
                            <p class="mb-0"><?php _e(' example', 'lsdcommerce');?> : <strong>bank_</strong>syariah_indonesia</p>
                        </div>

                        <div class="form-group">
                            <label class="form-label" for="name">
                                <?php _e('Bank Name (Required)', 'lsdcommerce');?>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="form-label" for="logo">
                                <?php _e('Bank Logo (Required)', 'lsdcommerce');?>
                            </label>
                        ...
                        <div class="divider text-center" style="margin-top:25px;" data-content="<?php _e('Bank Account', 'lsdcommerce');?>"></div>

                        <div class="form-group">
                            <label class="form-label" for="bank_code">
                                <?php _e('Bank Code (Optional)', 'lsdcommerce');?>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="form-label" for="swift_code">
                                <?php _e('SWIFT Code (Optional)', 'lsdcommerce');?>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="form-label" for="account_number">
                                <?php _e('Account Number (Required)', 'lsdcommerce');?>
                            </label>
                        ...
                        <div class="form-group">
                            <label class="form-label" for="account_owner">
                                <?php _e('Account Owner (Required)', 'lsdcommerce');?>
                            </label>
...
/**
             ...
             * Make it Relative
             */
            do_action("lsdcommerce/admin/payment/custombank/{$this->pointer}");
            ?>
                        ...
                        <div class="form-group">
                            <label class="form-label" for="instruction">
                                <?php _e('Payment Instruction', 'lsdcommerce');?>
                            </label>
                            <textarea class="form-input" name="instruction" placeholder="<?php _e('Please make payments to this account according to the total', 'lsdcommerce');?>" lsd-rows="3"><?php esc_attr_e($settings['instruction']);?></textarea>
                        </div>
...
$forms = apply_filters("lsdcommerce/form/fields/payment", array());
            ...
            ?>
                        ...
                        <button class="btn btn-light btn-block lsdc-payment-delete" style="background:#e85600;color:#fff;border:none;" id="<?php echo $payment_id; ?>_delete">
                            <?php _e('Delete', 'lsdcommerce');?>
                        </button>
                        <?php endif;?>
                ...
                <div class="panel-footer">
                    <button class="btn btn-primary btn-block lsdc-payment-save" id="<?php echo $payment_id; ?>_payment">
                        <?php _e('Save', 'lsdcommerce');?>
                    </button>
                </div>
            </div>
        <?php
            ...
            ?>
                ...
                <div class="lsdp-mb-5">
                    ...
                    <?php if (!empty($settings['swift_code'])): ?>
                        <p class="mb-0 lsdp-font-13 pb-5"><?php _e('SWIFT Code', 'lsdcommerce');?></p>
                        <h6 class="title font-weight-medium lsdp-mb-10"><?php echo esc_attr($settings['swift_code']); ?></h6>
                    <?php endif;?>

                    <!-- Account  -->
                    <p class="mb-0 lsdp-font-13 pb-0"><?php _e('Account Number ', 'lsdcommerce');?></p>
                    <div class="lsdp-row no-gutters">
                        ...
                        <div class="col-auto ml-auto">
                            <a onclick="lsdcCopy('#account', this)" class="form-title text-dark-grey c-pointer">
                                <?php _e('Copy', 'lsdcommerce');?>
                    ...
                    <div class="lsdp-mb-15 lsdp-mt-10 pb-0">
                        <p class="mb-0 lsdp-font-13"><?php _e('Account Owner ', 'lsdcommerce');?></p>
        ...
        public function notification_text(object $report_id, string $event, string $gateway)
        {
            ...
            if (isset($settings['swift_code']) && $settings['swift_code'] != '') {
                $template .= __("SWIFT Code", 'lsdcommerce') . ' : ' . esc_attr($settings['swift_code']) . PHP_EOL;
            }
            $template .= __("Account Number", 'lsdcommerce') . ' : ' . '(' . intval($settings['bank_code']) . ') ' . abs($settings['account_number']) . PHP_EOL;
            $template .= __("Account Owner", 'lsdcommerce') . ' : ' . esc_attr($settings['account_owner']) . PHP_EOL . PHP_EOL;
        ...
        public function notification_html(object $report_id, string $event, string $gateway)
        {
            ...
            $swift = isset($settings['swift_code']) && $settings['swift_code'] != '' ? '<p style="margin:0;padding:0;">' . __("SWIFT Code", 'lsdcommerce') . ' : ' . esc_attr($settings['swift_code']) . '</p>' : '';
            ...
            $swift .
            '<p style="margin:0;padding:0;">' . __("Account Number", 'lsdcommerce') . ' : ' . '(' . abs($settings['bank_code']) . ') <strong>' . abs($settings['account_number']) . '</strong></p>
                        <p style="margin:0;padding:0;">' . __("Account Owner", 'lsdcommerce') . ' : <strong>' . esc_attr($settings['account_owner']) . '</strong></p>
                    </div>
                </td>
            </tr>
            <tr>
                <td align="left" style="font-size:0px;padding:0px 25px;">
                    <div
                        ...
                        <h6>' . __('Instruction', 'lsdcommerce') . '</h6>
